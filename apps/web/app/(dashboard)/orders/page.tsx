...
import { useCreateOrder, useOrderAction, useOrders, type OrderStatus } from '@/hooks/useOrders';

const ADMIN_ACTION: Partial<Record<OrderStatus, { action: 'confirm' | 'dispatch' | 'deliver'; label: string }>> = {
  PENDING: { action: 'confirm', label: 'Confirm' },
              ...
              {data.map((o) => {
                ...
                return (
                  <TR key={o.id}>
                    ...
                    <TD className="max-w-xs truncate text-muted-foreground">{o.items.map((i) => `${i.product.name} ×${Number(i.confirmedQuantity ?? i.requestedQuantity)}`).join(', ')}</TD>
                    ...
                    <TD className="text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        {!next && !canCancel && <span className="text-caption text-muted-foreground">—</span>}
                      </div>
                    </TD>
                  </TR>
                );

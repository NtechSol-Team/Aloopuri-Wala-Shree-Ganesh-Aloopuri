...
export type PeriodKey = 'today' | 'yesterday' | 'month' | 'lastMonth' | 'all' | 'custom';

export const PERIODS: Array<[PeriodKey, string]> = [
  ['today', 'Today'], ['yesterday', 'Yesterday'], ['month', 'This Month'],
  ['lastMonth', 'Last Month'], ['all', 'All'], ['custom', 'Custom Range'],
];
...
export function periodRange(period: PeriodKey, custom: { from: string; to: string }): { from?: string; to?: string } {
  ...
  switch (period) {
    ...
    case 'lastMonth': return { from: iso(new Date(n.getFullYear(), n.getMonth() - 1, 1)), to: iso(new Date(n.getFullYear(), n.getMonth(), 0)) };
